...
class TestPekatVisionInstance(TestCase):
    ...
    def test_recognise(self):
        ...
        # response type
        # annotated_image
        img, _ = p.recognise(self.img_nok_path, response_type='annotated_image')
        img_template = cv2.imread(self.img_nok_annotated_path)
        if cv2.subtract(img, img_template).sum() != 0:
            self.fail()
        # heatmap
        img, _ = p.recognise(self.img_nok_path, response_type='heatmap')
        img_template = cv2.imread(self.img_nok_heatmap_path)
        if cv2.subtract(img, img_template).sum() != 0:
            self.fail()
        # context
        p.recognise(self.img_nok_path, response_type='context')
        p.recognise(self.img_nok_path)

        # data in request
        r = p.recognise(self.img_nok_path, data='test')
    ...
    def test_stop(self):
        ...
        p.stop()
        try:
            p.recognise(self.img_nok_path, timeout=4)
